...
const fetchAndCacheCity = async (email: string, lat: string, long: string): Promise<string | Error> => {
    try {
        const response = await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${long}&key=${process.env.GOOGLE_API_KEY}`);
...
const fetchAndCacheBounds = async (city: string): Promise<void> => {
    try {
        ...
        // Cache bounds data
        await setToCache(`${city}`, JSON.stringify(bounds), CACHE_BOUNDS_EXPIRES);
    } catch (error: any) {
...
export const helperCacheFunctionCity = async (lat: string, long: string): Promise<string | Error> => {
    ...
    try {
        ...
        if (!city) {
            // Fetch and cache city data if not available in cache (a.k.a cache miss)
        ...
        const cachedBounds = await getFromCache(city);
        if (!cachedBounds) {
            // Fetch and cache bounds data if not available in cache (cache miss)
        ...
        const isInside = isCoordinateInBBox(parseInt(lat), parseInt(long), JSON.parse(cachedBounds || ""));

        if (isInside) {
            return city;
        } else {
            // This means that the user is no longer within the city and therefore re-fetch the city and cache
            const newCity = await fetchAndCacheCity(email, lat, long)
            if (newCity instanceof Error) {
                throw newCity; // Propagate the error if needed
            }
